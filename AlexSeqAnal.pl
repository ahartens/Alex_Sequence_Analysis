...
#     if ($file =~ /\.$file_type$/){
    if ($file =~ /\.seq$/){
        ...
        if ($sequence =~ m/[^acgt]/){
            ...
            push (@sequences, $sequence);
            insert_id($sequence, $const_frame_leader_all, $const_frame_leader_all_revcomp, $constant_vector_gamma, $constant_vector_kappa,
$constant_vector_lamda, $const_frame_constant_gamma, $const_frame_constant_kappa, $const_frame_constant_lamda);	
        }
        close (FILE);
    }
}
